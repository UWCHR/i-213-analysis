...
author:
- "[Andrew Shaw](https://github.com/andrew-b-shaw)"
date: 29 April, 2024
output:
    html_document:
        html_preview: true
...
2. Arrests or Processing at Check-Ins/Self-Report (FALSE)
    a. Arrests at Check-Ins/Self-Report
        i. Referred by Border Patrol
        ii. Not Referred by Border Patrol
    b. Processing at Check-Ins/Self-Report
        i. Referred by Border Patrol
        ii. Not Referred by Border Patrol
3. Miscategorized (FALSE):
    a. Transfers of Custody from State Prison/Federal Prison/USMS
...
- Test Accuracy: `r test_results$test_acc`
- Positive Precision: `r test_results$pos_precision`
- Negative Precision: `r test_results$neg_precision`
- Positive Recall: `r test_results$pos_recall`
- Negative Recall: `r test_results$neg_recall`
- Positive F1: `r test_results $pos_f1`
- Negative F1: `r test_results $neg_f1`
...
```{r}
cca_counts <- cca %>% count(lawsuit, method_short, cca_label, name="count")

cca_counts_all <- cca %>% count(lawsuit, cca_label, name="count") %>% 
...
We then join our results with our other I-213 analysis data to investigate any possible correlations between contemporary community arrests and other variables. Since the lawsuit associated with each record acts as a proxy for time, we also segment by lawsuit when appropriate.
...
```{r}
i213 <- inner_join(i213, cca %>% select(file_hash,
                                        narrative_length,
                                        hand_label,
                                        model_label,
                                        is_hand_labeled,
                                        cca_label),
                   by="file_hash") %>% 
  mutate(lawsuit=paste("Lawsuit", lawsuit)) # mutate for facet wrap formatting
...
Second, we analyze correlations between country of origin, sex, and age with contemporary community arrests. Based on our analysis, males, those in the age range of 25 to 40 years, and immigrants from Mexico are are more likely to be subject to contemporary community arrests. We also observe an overall increase in diversity of country of origin for arrest records from lawsuit 2, especially for non-contemporary community arrests.
...
i213 %>% count(lawsuit, cca_label, sex) %>% 
  ggplot(aes(x=cca_label, y=n, fill=sex)) +
...
i213 %>% count(lawsuit, cca_label, age_group) %>% 
  ggplot(aes(x=cca_label, y=n, fill=age_group)) +
  geom_col(position="stack") +
  facet_wrap(~lawsuit) +
...
i213 %>% count(lawsuit, cca_label, country_short) %>% 
...
### Geography
Third, we analyze correlations between geographical location and contemporary community arrests. In general, we find that the Seattle office performs the lowest proportion of contemporary community arrests out of ICE offices in the Pacific Northwest. We also observe a significant decrease in contemporary community arrests from Lawsuit 1 to Lawsuit 2.
...
map_plot <- ggplot(data=world) +
    ...
    geom_sf(data=map, aes(size=n, fill=cca_label), shape=21, alpha=.5, show.legend=FALSE) +
    pnw_coords +
...
i213 %>% count(office, lawsuit, cca_label) %>% 
