...
p1 <- i213 %>% 
  ...
  facet_wrap(~lawsuit)  +
  labs(title = "I-213 Method of location/apprehension dist.",
       subtitle = "Comparison of lawsuit productions")
  
p1

```

These codes do not always correspond with local custody; may also involve DOC custody.

```{r CLC_lawsuit_compare}

p1 <- i213 %>% 
  filter(source == 'ice',
         state == 'WA') %>% 
  mutate(clc = case_when(method_short == "CLC" ~ TRUE,
                         TRUE ~ FALSE)) %>% 
  count(lawsuit, county, clc) %>% 
  ggplot(aes(x = n, y=county, fill=clc)) +
  geom_col() +
  facet_wrap(~lawsuit)  +
  labs(title = "I-213 Method of location/apprehension dist.",
       subtitle = "Comparison of lawsuit productions")
...
p3 <- i213 %>% 
  filter(source == 'ice',
         state == 'WA',
         method_short == 'CLC') %>% 
  count(lawsuit, county, method_short) %>% 
  ggplot(aes(x = n, y=county, fill=method_short)) +
  geom_col() +
  facet_wrap(~lawsuit)  +
  labs(title = "I-213 Method of location/apprehension dist.",
       subtitle = "Comparison of lawsuit productions")
...
p2 <- i213 %>%
  filter(date >= '2019-01-01',
         ...
         state %in% c('WA', 'OR'),
         !is.na(crim_record)) %>% 
  count(crim_record, method_short, fy) %>% 
  ggplot(aes(x=as.factor(fy), y=n, fill=crim_record)) +
  geom_col(position='fill') +
  facet_wrap(~method_short)
...
# Mentions of prosecutorial discretion relatively steady

p1 <- i213 %>% 
  filter(date >= '2019-01-01',
         date <= '2021-11-30') %>% 
  count(month, mentions_prosecutorial_disc) %>% 
  ggplot(aes(x=month, y=n, fill=mentions_prosecutorial_disc)) +
  geom_col()

p1

# "Self-report" or "as instructed" appearances increased greatly

p2 <- i213 %>% 
  filter(date >= '2019-01-01',
         date <= '2021-11-30') %>% 
  count(month, mentions_self_report) %>% 
  ggplot(aes(x=month, y=n, fill=mentions_self_report)) +
  geom_col()

p2

# Mentions of detainer not clear trend, note includes cases where detainer lodged but not honored

p3 <- i213 %>% 
  filter(date >= '2019-01-01',
         date <= '2021-11-30') %>% 
  count(month, mentions_detainer) %>% 
  ggplot(aes(x=month, y=n, fill=mentions_detainer)) +
  geom_col()

p3

# Proportion of records mentioning "jail" down

p4 <- i213 %>% 
  filter(date >= '2019-01-01',
         date <= '2021-11-30') %>% 
  count(month, mentions_jail) %>% 
  ggplot(aes(x=month, y=n, fill=mentions_jail)) +
  geom_col()
...
p5 <- i213 %>% 
  filter(date >= '2019-01-01',
         date <= '2021-11-30') %>% 
  count(month, mentions_secure_comm) %>% 
  ggplot(aes(x=month, y=n, fill=mentions_secure_comm)) +
  geom_col()
...
p6 <- i213 %>% 
  filter(date >= '2019-01-01',
         date <= '2021-11-30') %>% 
  count(month, mentions_sheriff) %>% 
  ggplot(aes(x=month, y=n, fill=mentions_sheriff)) +
  geom_col()

p6

# Greater proportion of records mention "prison" during height of COVID

p7 <- i213 %>% 
  filter(date >= '2019-01-01',
         date <= '2021-11-30') %>% 
  count(month, mentions_prison) %>% 
  ggplot(aes(x=month, y=n, fill=mentions_prison)) +
  geom_col()
...
p8 <- i213 %>% 
  filter(date >= '2019-01-01',
         date <= '2021-11-30') %>% 
  count(month, mentions_airport) %>% 
  ggplot(aes(x=month, y=n, fill=mentions_airport)) +
  geom_col()
...
p9 <- i213 %>% 
  filter(date >= '2019-01-01',
         date <= '2021-11-30') %>% 
  count(month, mentions_surveillance) %>% 
  ggplot(aes(x=month, y=n, fill=mentions_surveillance)) +
  geom_col()
